...
- 구현: `computeCentroid`(`meanVector` 재사용, 첫 유효 벡터 차원 기준) · `clusterArticles` 종료 시 보정(결과에 `centroidBackfilled`) · `publishIssue` 는 `AdminStore.hasCentroid` 가 false 일 때만 `recomputeCentroid` 를 부른다.
...
- 구현: `AdminStore.mergeIssue`(Prisma 는 `$transaction` + `PIPELINE_TRANSACTION_OPTIONS`) · `listMergeTargets(excludeIssueId)`(최근 30일) · 금지 규칙은 `AdminMessage.ERROR_MERGE_SELF`/`_TARGET_NOT_FOUND`/`_TARGET_REJECTED`/`_SOURCE_PUBLISHED` · 화면은 검수 폼 분류 카드 아래 `IssueMergeFormView`(폼 필드 `targetIssueId`) · 메모 문구는 `src/server/mergeReviewNotes.ts` 한곳에서 만든다.
