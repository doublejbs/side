...
7. **언론은 어떻게 다르게 보도했을까요?** — 서브텍스트 `{coveragePeriodLabel} · {mediaOutletCount}개 매체 · 분석 기사 {합}건`(상단 통계 타일의 "원문 기사"는 요약의 핵심 출처 수로, 서로 다른 값이다) + 진보/중도/보수 카드(기사 수, 주요 프레임, 키워드 칩, 대표 기사) + 브랜드색 "공통적으로 다룬 내용" 카드
