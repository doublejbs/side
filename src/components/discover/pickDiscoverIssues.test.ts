...
const issues = getIssues().map(toIssueSummary);
...
describe('pickMostDifferentIssue', () => {
  ...
  it('내 선택의 전체 퍼센트가 가장 낮은 이슈를 고른다', () => {
    ...
    expect(pickMostDifferentIssue(issues, votes)?.id).toBe('ai-regulation');
  });
  ...
  it('아직 모르겠음 선택은 unsure 퍼센트로 비교한다', () => {
    ...
    expect(pickMostDifferentIssue(issues, votes)?.id).toBe('work-week-4-5');
  });
...
describe('pickMostDividedIssue', () => {
  it('찬반 차이가 가장 작은 이슈를 고른다', () => {
    ...
    expect(picked?.id).toBe('nuclear-expansion');
